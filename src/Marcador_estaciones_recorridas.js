...
function procesarCSVEstaciones(content) {
  ...
  for (let i = 1; i < filas.length; i++) {
    ...
    nombre2idx.set(normalizaNombre(nombre), idxMesh);
    nombre2idx.set(normalizaSimple(nombre), idxMesh);
  }
}
...
function encontrarIdx(nombre) {
  const n1 = normalizaNombre(nombre);
  const n2 = normalizaSimple(nombre);
  if (nombre2idx.has(n1)) return nombre2idx.get(n1);
  if (nombre2idx.has(n2)) return nombre2idx.get(n2);
  return null;
}
